...
impl Field {
    ...
    pub fn merge_from(&mut self,
        ...
        match field.field_type(descriptors) {
            ...
            Group => unimplemented!(),
            UnresolvedEnum(e) => Err(serde_protobuf_error::ErrorKind::UnknownEnum(e.to_owned()).into()),
            UnresolvedMessage(m) => Err(serde_protobuf_error::ErrorKind::UnknownMessage(m.to_owned()).into()),
        }
    }
              ...
              R: Fn(&mut protobuf::CodedInputStream<'a>) -> protobuf::ProtobufResult<A>
    {
        if expected_wire_type == actual_wire_type {
            ...
            Err(serde_protobuf_error::ErrorKind::BadWireType(actual_wire_type).into())
        }
    }
